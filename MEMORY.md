...
| the-repo | rimajj/LPJFIT_Emulator IS this project's repo. Never re-ask where to push. Its `main` holds the PREDECESSOR project and this history has its own root commit, so the two share no commit. | owner, standing | 2026-09-08 |
| push-names-taken | Consequence of the-repo: `main` and `line/X` on the remote hold the predecessor's branches, so those two pushes are refused as non-fast-forward. `line/D` and `line/T` were free and are pushed. | measured | 2026-09-08 |
