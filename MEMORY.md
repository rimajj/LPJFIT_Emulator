...
| origin-is-predecessor | `origin` points at the PREDECESSOR's GitHub repo (rimajj/LPJFIT_Emulator) and shares NO common ancestor with this history, so tools/merge.sh cannot run and nothing is pushed. Needs an owner decision. | measured | 2026-09-08 |
| ci-never-ran | Nothing has ever been pushed (see origin-is-predecessor), so all nine gates in .github/gates.toml have never run on any commit. The local tools/ checkers are the only evidence a commit is clean. | measured | 2026-09-08 |
...
| lint-red-on-main | `ruff format --check .` (the `lint` gate) would reformat 10 tracked files on main as committed; ruff is pinned only >=0.6 while CI installs latest (0.16). Unresolved: version drift vs neglect. | measured | 2026-09-08 |
