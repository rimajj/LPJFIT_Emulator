...
WHY IT MATTERS HERE SPECIFICALLY. This project needs a GitHub token to poll its own CI, and the
token
lives OUTSIDE every worktree on purpose (in the user's `gh` config), so rotating it covers every
        ...
        for i, ln in enumerate(lines, start=1):
            for label, rx in PATTERNS:
                ...
                rep.add(
                    ...
                    line=i,
                    hint=(
                        "keep credentials outside every worktree — a pushed token is auto-revoked, "
                        "which breaks CI for all lines at once"
                    ),
                )
    return rep.emit()
