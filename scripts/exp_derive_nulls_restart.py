...
10.0 %, so a REAL restart compared year-to-year against its own starting state fails the criterion in
71 % of cells. The drift of the state itself -- the 1000-year spin-up trend, +6.8 %/century median --
...
THE ARMS, AND WHY EACH IS A COMPETITOR RATHER THAN A STRAW MAN. Every null here is a real, valid,
loadable restart file for the same 20 cells -- a thing you could hand the model instead of running
the emulator at all:

  no_relaxation        the analytic arm: the model does not move the state, so its year-20 score is
                       its year-0 score. If the measured year-20 score is not better than this, then
                       running the model forward does not repair the emitted state, and the "short
                       polish run" fallback in the roadmap is dead.
  ...
  climatological_mean  the mean state over the scored cells. Not a loadable restart -- year-0 only,
                       and it is declared as such rather than quietly scored at year 20.
  shuffled_target      each cell's emulated state paired with another target cell's control.
...
def drift_reference(version: str) -> dict[str, float]:
    ...
    """
    frame = pl.read_parquet(Path(str(paths()["scratch"]["corpus"])) / version / "spin_convergence.parquet")
...
def oof_nulls(
    ...
    out = {k: np.full_like(corpus_state, np.nan) for k in
           ("geographic_address", "nearest_analogue", "climatological_mean", "shuffled_target")}
    for i in range(n):
...
def main() -> int:
    ...
    for arm, (run, rel) in CONTROL.items():
        states[arm], bases[arm] = state_of(run, rel, cells=20, nproc=args.nproc)
    for variant, (run, y0, y1) in VARIANTS.items():
        states[f"{variant}_y0"], bases[f"{variant}_y0"] = state_of(
            run, f"restart/{y0}", cells=20, nproc=args.nproc
        )
        states[f"{variant}_y1"], bases[f"{variant}_y1"] = state_of(
            run, f"restart/{y1}", cells=20, nproc=args.nproc
        )
    ...
    report["cells"] = {"first": first_cell, "n": int(n_sub), "note": "20 of 54,020 tree-bearing cells"}
    ...
    emulated = {
        v: (matrix(states[f"{v}_y0"], SCORED_CONJUNCTIVE), matrix(states[f"{v}_y1"], SCORED_CONJUNCTIVE))
        for v in VARIANTS
    }
    ...
    def mean_per_quantity(
        ...
        cell-quantities instead of demanding all 22 at once is what buys the resolution to tell
        "the model repaired the state" from "the model wrecked it". The conjunctive number is
    ...
    for bname, band in bands.items():
        arms = {n: band_frac_conjunctive(p, control0, band) for n, p in preds.items()}
        # The analytic arm: the model does not move the state at all, so the emitted year-0 state is
        # scored against a truth that HAS moved. That is the comparison the 20-year run repeats, and
        # it is the null the model arm has to beat for "run it forward and it relaxes" to be true.
        entry: dict[str, object] = {
            "nulls_conjunctive": arms,
        ...
        for variant, (e0, e1) in emulated.items():
            entry[variant] = {
                ...
                # running the model forward moved the state AWAY from the truth.
        ...
        for n, v in sorted(arms.items(), key=lambda kv: -kv[1]):
            print(f"  {n:24s} {v:.4f}  {entry['nulls_mean_per_quantity'][n]:.4f}")  # type: ignore[index]
        print(
        ...
        for variant in VARIANTS:
            e = entry[variant]
            better = e["mean_per_quantity_year1"] - e["mean_per_quantity_frozen_vs_year1_truth"]  # type: ignore[index]
            verdict = "model REPAIRS it" if better > 0 else "model moves it AWAY"
            print(
                f"  {variant:14s} {e['mean_per_quantity_year0']:8.4f} "  # type: ignore[index]
                f"{e['mean_per_quantity_year1']:8.4f} "  # type: ignore[index]
                f"{e['mean_per_quantity_frozen_vs_year1_truth']:8.4f}   {verdict} ({better:+.4f})"  # type: ignore[index]
            )
